...
export default function ChromaDbStoreNode({ id, data, selected }: HubNodeProps) {
  ...
  return (
    <div className={`rounded-lg border-2 border-slate-900 px-4 py-3 bg-sky-600 min-w-[170px] shadow-sm text-white relative ${selected ? "ring-4 ring-slate-900" : statusRing}`}>
      <Handle
        type="target"
        position={Position.Left}
        className="!w-3.5 !h-3.5 !bg-slate-900 !border-2 !border-white hover:!scale-125 transition-transform"
      />
      <div className="text-[10px] font-bold uppercase tracking-wider text-sky-100">Vector Storage</div>
      <div className="text-xs font-bold text-white">{data.label || "ChromaDB Store"}</div>
      <Handle
        type="source"
